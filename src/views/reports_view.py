...
class ReportsView(QWidget):
    ...
    def update_report(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Error al generar el informe: {str(e)}")
    ...
    def show_sales_report(self):
        ...
        for row, sale in enumerate(sales):
            ...
            self.table.setItem(
                row, 1, QTableWidgetItem(f"${sale['total']:.2f}"))
            self.table.setItem(row, 2, QTableWidgetItem(
                str(sale['cantidad_productos'])))
    ...
    def show_stock_movements_report(self):
        ...
        start_date = self.start_date.date().toString("yyyy-MM-dd")
        end_date = self.end_date.date().toString("yyyy-MM-dd")
        movements = self.inventory_service.get_stock_movements_by_period(
            start_date, end_date)
    ...
    def show_inventory_value_report(self):
        ...
        for row, category in enumerate(inventory_value):
            ...
            self.table.setItem(row, 1, QTableWidgetItem(
                str(category['cantidad'])))
            self.table.setItem(row, 2, QTableWidgetItem(
                f"${category['valor_total']:.2f}"))

    def generate_report(self):
        """Generate the current report."""
        self.update_report()
        QMessageBox.information(
            self, "Éxito", "Informe generado correctamente")
    ...
    def export_report(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "Éxito", "Informe exportado correctamente")
        except Exception as e:
            QMessageBox.critical(
                self, "Error", f"Error al exportar el informe: {str(e)}")
