...
class RegisterWindow(QDialog):
    ...
    def setup_ui(self):
        ...
        title.setStyleSheet(
            f"color: {APP['theme']['primary_color']}; font-size: 24px; font-weight: bold;")
        ...
        try:
            ...
            if success:
                QMessageBox.information(
                    self, "Éxito", "Usuario registrado correctamente")
                self.accept()
            else:
                QMessageBox.warning(
                    self, "Error", "No se pudo registrar el usuario")
        except Exception as e:
            QMessageBox.critical(
                self, "Error", f"Error al registrar usuario: {str(e)}")
